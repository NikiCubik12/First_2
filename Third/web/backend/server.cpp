...
public:
    ...
    string concat() 
    {
        ...
        if (seq1.isBit != seq2.isBit) {
            return "❌ Ошибка: Нельзя объединить BitSequence с обычной последовательностью";
        }
        
        string type1 = seq1.getTypeString();
        string type2 = seq2.getTypeString();
        
        string baseType1 = type1;
        string baseType2 = type2;
        
        size_t pos1 = baseType1.find(" (");
        size_t pos2 = baseType2.find(" (");
        if (pos1 != string::npos) baseType1 = baseType1.substr(0, pos1);
        if (pos2 != string::npos) baseType2 = baseType2.substr(0, pos2);
        
        if (baseType1 != baseType2 && !seq1.isBit && !seq2.isBit) {
            return "❌ Ошибка: Нельзя объединить " + baseType1 + " с " + baseType2 + 
                   ". Можно объединять только последовательности одного типа";
        }
        
        bool isFirstMutable = (type1.find("Mutable") != string::npos);
        bool isSecondMutable = (type2.find("Mutable") != string::npos);
        ...
        else 
        {
            ...
            bool resultShouldBeImmutable = !isFirstMutable || !isSecondMutable;
            
            if (resultShouldBeImmutable) 
            {
        ...
        string resultType = concatResult.getTypeString();
        string warning = "";
        if ((!isFirstMutable || !isSecondMutable) && !seq1.isBit) {
            warning = " ℹ️ Результат Immutable - создаётся новая неизменяемая последовательность";
        }
        
        return "✅ Результат объединения: " + resultStr + " (тип: " + resultType + ")" + warning;
    }
